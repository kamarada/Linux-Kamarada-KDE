...
image_file='image/Linux_Kamarada_42.2_KDE.x86_64-0.0.15.raw'
image_arch='x86_64'
schema_ver='5.2'
...
if ! [ -d "$src/" ] || ! [ -f "$src/config.xml" ]; then
  if [ -f "$dir/source.kiwi.txz" ]; then
    echo "Found source.kiwi.txz, extracting ..."
    mkdir $dir/source/
    tar -xJf source.kiwi.txz -C source/
    echo "Successfully extracted source.kiwi.xz archive"
  else
    printf "%s: %s\n" \
      "$0" "Cannot find appliance archive." \
